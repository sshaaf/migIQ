...
This directory contains scripts for evaluating the `mig-containerize` skill.
...
Run your evals in a workspace directory (e.g., `mig-containerize-workspace/iteration-1/`):
...
```bash
cd mig-containerize/evals
python grade_outputs.py ../../mig-containerize-workspace/iteration-1 eval-1-vague-java
```
...
```bash
cd mig-containerize/evals
python aggregate_benchmark.py ../../mig-containerize-workspace/iteration-1 1 ../../benchmark-results
```
...
git commit -m "Add benchmark results for mig-containerize iteration 1"
git push
```
