...
This directory contains scripts for evaluating the `migiq` skill.
...
Run your evals in a workspace directory (e.g., `migiq-workspace/iteration-1/`):
...
```bash
cd migiq/evals
python grade_outputs.py ../../migiq-workspace/iteration-1 eval-1-vague-java
```
...
```bash
cd migiq/evals
python aggregate_benchmark.py ../../migiq-workspace/iteration-1 1 ../../benchmark-results
```
...
git commit -m "Add benchmark results for migiq iteration 1"
git push
```
