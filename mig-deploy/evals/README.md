...
This directory contains scripts for evaluating the `mig-deploy` skill.
...
Run your evals in a workspace directory (e.g., `mig-deploy-workspace/iteration-1/`):
...
```bash
cd mig-deploy/evals
python grade_outputs.py ../../mig-deploy-workspace/iteration-1 eval-1-vague-java
```
...
```bash
cd mig-deploy/evals
python aggregate_benchmark.py ../../mig-deploy-workspace/iteration-1 1 ../../benchmark-results
```
...
git commit -m "Add benchmark results for mig-deploy iteration 1"
git push
```
